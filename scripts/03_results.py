...
for filename, title, sections in reports:
    ...
    html += f"<h1>{title}</h1>"
    html += f"<p>O*NET 30.3; {len(occupations):,} occupations and {len(tasks):,} tasks. I asked Claude Fable to score work activities using a mid-2026 capability rubric. Exposure measures capability overlap, not employment loss or observed adoption.</p>"
    html += "<p>Task scores are importance × relevance weighted means on a 0–1 scale. I compare modern LLM judgments with Felten’s earlier researcher-designed exposure measure. In the secondary hybrid, I blend Claude judgments and rescaled ability exposure equally. Correlations describe agreement on matched occupations, not predictive accuracy or a pure LLM-versus-human effect: the approaches differ in measurement unit and capability vintage. Percentile changes are fractions of the ranking distribution; 0.10 means ten percentile points.</p>"
    html += "<p>I use the saved activity scores to generate these reports. Shared O*NET inputs and possible model familiarity with the research mean agreement is not independent validation.</p>"
