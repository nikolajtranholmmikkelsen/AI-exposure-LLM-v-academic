"""Test zero exposure for tasks containing physical language, alone and with the broad hierarchy."""
...
def match_rules(text, patterns):
    return [(row.rule_id, match.group(0)) for row in patterns.itertuples()
            for match in re.finditer(row.pattern, text, flags=re.IGNORECASE)]


def run():
    OUT.mkdir(parents=True, exist_ok=True)
    tasks = pd.read_csv(ROOT / "results/robustness/task_overrides.csv")
    ...
    patterns = pd.read_csv(ROOT / "data/rubric/physical_task_patterns.csv")
    matches = tasks.Task.map(lambda text: match_rules(text, patterns))
    tasks["matched_rule_ids"] = matches.map(lambda items: "; ".join(dict.fromkeys(i[0] for i in items)))
    tasks["matched_text"] = matches.map(lambda items: "; ".join(i[1] for i in items))
    tasks["text_flag"] = matches.map(bool)
    tasks["combined_flag"] = tasks.text_flag | tasks.broad_flag
    denominators = tasks.groupby("O*NET-SOC Code").weight.sum()
    occupations = original[["O*NET-SOC Code", "Title", "exposure"]].rename(columns={"exposure": "original"})
    stats = []
    for rule in ["text", "combined"]:
    ...
    adjusted6 = occupations.groupby("soc6")[["text", "combined"]].mean().round(4)
    ...
    fig.savefig(OUT / "text_override_comparison.png", dpi=200, facecolor="white")
    plt.close(fig)
